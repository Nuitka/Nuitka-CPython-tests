...
class BytesFwalkTests(FwalkTests):
    ...
    # Nuitka: Only the wrapper generator around fwalk leaks FDs on close
    # with CPython 3.12.0 to 3.12.3, which was fixed in CPython 3.12.4.
    @unittest.skipIf(
        sys.version_info < (3, 12, 4),
        "CPython 3.12.0-3.12.3 leak the fwalk FDs on close (gh-89727)",
    )
    def test_fd_finalization(self):
        FwalkTests.test_fd_finalization(self)
